...
Planning (repeatable; replan novel/arc/chapter as needed before generation):

```text
plan novel -> novel_plan.json (arc structure)
plan arc N or plan arc N-M -> arc_plan.json (scene beats for each arc in range)
...
- `plan novel`:
  ...
  - Planning commands are repeatable: re-running `plan novel`, `plan arc`, or `plan chapter` will regenerate and overwrite the corresponding plan files. Implementations SHOULD preserve previous versions (timestamped or `.bak`) so users can review or roll back earlier plans.

- `plan arc N` or `plan arc N-M`:
  - Generates `arcs/arc_N/arc_plan.json` (or `arc_M` for each arc in range) from `novel_plan.json` + constitution + world.
  - Defines scene beats and chapter breakdown for each arc in the specified range.
...
/novel/
  ...
  arcs/
    arc_01/
      arc_plan.json        ← generated by `plan arc 1` (or `plan arc 1-2` for a range)
      ch_001_plan.json
      ch_002_plan.json
      ch_003_plan.json
  chapters/
...
4. Proceed to generation when satisfied with plan.
  Planning is iterative: users may re-run `plan novel`, `plan arc`, or `plan chapter` multiple times to refine structure. Replanning will overwrite the active plan files; implementations SHOULD keep prior versions for audit/rollback.
...
# 7) Plan the arc (single or range)
python run.py plan arc 1-2
...
- `plan novel`, `plan arc`, `plan chapter` regenerate only their scope and may be re-run to refine plans; prior plan versions should be preserved by the implementation when possible.
